...
// ─── PLAYER DATA ───────────────────────────────────────────────────────────────
// Each answer: { c: correct (bool), t: time taken in seconds }
const PLAYERS_RAW = [
  {
    id: 1, name: "NEON_VIPER", avatar: "🐍", isHost: true,
    answers: [
      {c:true,t:1.1},{c:true,t:1.3},{c:true,t:1.0},{c:true,t:1.5},{c:true,t:0.9},
      {c:true,t:1.2},{c:true,t:1.4},{c:true,t:1.1},{c:true,t:1.6},{c:true,t:1.0},
      {c:true,t:1.3},{c:true,t:1.2},{c:true,t:0.8},{c:true,t:1.1},{c:true,t:1.3},
    ],
  },
  {
    id: 2, name: "ALEX", avatar: "🤖", isHost: false,
    answers: [
      {c:true,t:1.8},{c:true,t:2.1},{c:true,t:1.5},{c:false,t:15},{c:true,t:1.9},
      {c:true,t:1.7},{c:true,t:2.0},{c:true,t:1.6},{c:true,t:2.2},{c:true,t:1.8},
      {c:true,t:1.5},{c:true,t:1.9},{c:true,t:2.0},{c:false,t:15},{c:true,t:1.7},
    ],
  },
  {
    id: 3, name: "MIKA", avatar: "👾", isHost: false,
    answers: [
      {c:true,t:2.1},{c:true,t:2.3},{c:false,t:15},{c:true,t:2.0},{c:true,t:1.8},
      {c:true,t:2.5},{c:true,t:2.2},{c:false,t:15},{c:true,t:2.1},{c:true,t:2.4},
      {c:true,t:2.0},{c:true,t:1.9},{c:true,t:2.2},{c:true,t:2.1},{c:false,t:15},
    ],
  },
  {
    id: 4, name: "GRID_GHOST", avatar: "👻", isHost: false,
    answers: [
      {c:true,t:3.1},{c:false,t:15},{c:true,t:2.8},{c:true,t:3.2},{c:false,t:15},
      {c:true,t:2.9},{c:true,t:3.0},{c:true,t:2.7},{c:false,t:15},{c:true,t:3.1},
      {c:true,t:2.8},{c:false,t:15},{c:true,t:3.0},{c:true,t:2.9},{c:true,t:3.2},
    ],
  },
  {
    id: 5, name: "BYTE_BRAWLER", avatar: "🦾", isHost: false,
    answers: [
      {c:false,t:15},{c:true,t:4.2},{c:false,t:15},{c:true,t:3.8},{c:true,t:4.0},
      {c:false,t:15},{c:true,t:4.5},{c:true,t:3.9},{c:false,t:15},{c:true,t:4.1},
      {c:true,t:4.3},{c:false,t:15},{c:true,t:3.7},{c:false,t:15},{c:true,t:4.0},
    ],
  },
];
...
// ─── SCORING ────────────────────────────────────────────────────────────────────
const DMULT = { Easy: 1.0, Medium: 1.5, Hard: 2.0 };
const RANKS = [
...
function calcPlayer(p, diff) {
  const m = DMULT[diff];
  let score = 0;
  const correct = p.answers.filter((a) => a.c);
  p.answers.forEach(({ c, t }) => {
    if (!c) return;
    const base = 10 * m;
    const remaining = 15 - t;
    const speedBonus = Math.round((remaining / 15) * base);
    score += base + speedBonus;
  });
  score = Math.round(score);
  const acc = Math.round((correct.length / p.answers.length) * 100);
  const avg =
    correct.length > 0
      ? parseFloat((correct.reduce((s, a) => s + a.t, 0) / correct.length).toFixed(1))
      : null;
  const xp = score + (acc === 100 ? 50 : 0);
  return { ...p, score, acc, avg, xp, correctCount: correct.length };
}
...
// ─── COUNT-UP HOOK ──────────────────────────────────────────────────────────────
function useCountUp(target, duration = 1700, active = false) {
...
export default function MultiplayerResults() {
  ...
  const players = PLAYERS_RAW.map((p) => calcPlayer(p, MATCH.difficulty))
    .sort((a, b) =>
      b.score !== a.score
        ? b.score - a.score
        : b.acc !== a.acc
  ...
  const you = players.find((p) => p.id === YOU_ID);
  const { rank: rankInfo, pct: rankPct } = getRankInfo(you?.xp || 0);
  ...
  if (you?.avg !== null && (you?.avg ?? 99) < 2.0) achievements.push({ i: "⚡", n: "SPEED DEMON", d: "Average response under 2 seconds" });
  if (you?.correctCount === MATCH.totalQuestions) achievements.push({ i: "🧠", n: "TRIVIA MASTER", d: "Answered every question correctly" });
  const topAch = achievements.length > 0 ? achievements[0] : { i: "🎮", n: "PARTY UP", d: "Completed a multiplayer match session" };
      ...
      <div style={s.wrap}>
          ...
          <div style={s.matchPanel}>
            <div style={s.matchGrid}>
              {[
                ...
                ["DIFFICULTY", MATCH.difficulty.toUpperCase()],
                ["PLAYERS", PLAYERS_RAW.length],
                ["QUESTIONS", MATCH.totalQuestions],
              ].map(([lbl, val]) => (
        ...
        <div>
          ...
          <div style={s.statsGrid}>
            ...
            <StatCard label="AVG RESPONSE" value={ready && you?.avg !== null ? you?.avg : "—"} unit="s" accent="#C9562E" />
          </div>
        </div>
        ...
        <div>
          ...
          <div style={s.rankPanel}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={s.rankSub}>XP: {you?.xp}</span>
        ...
        <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap", paddingTop: 4 }}>
          <CyberButton label="◀ BACK TO LOBBY" onClick={() => alert("Returning to Lobby...")} />
          <CyberButton label="EXIT LOBBY ✕" onClick={() => alert("Disconnecting...")} red />
        </div>
      </div>
