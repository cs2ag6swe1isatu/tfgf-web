...
// ─── Palette ──────────────────────────────────────────────────────────────────
const C = {
  bg:"#010707", green:"#35E52B", greenGlow:"#3FFF56", greenShadow:"#0A3F0A",
  cyan:"#00DFFF", purple:"#BF5FFF", purpleGlow:"#D180FF", magenta:"#FF2D78",
  magentaGlow:"#FF6FA3", orange:"#FF8C00", orangeGlow:"#FFB347", gold:"#FFD700",
  goldGlow:"#FFE966", teal:"#00E5CC", tealGlow:"#4FFFEE", btnFill:"#10363A",
  btnText:"#2DE339", pixelGreen:"#3BE042", pixelCyan:"#00DFFF", pixelGreenDk:"#2BAA35",
};

// ─── All 10 ranks (simple 10k XP buckets) ─────────────────────────────────────
...
function AmbientPixels() {
  return (
    <div style={{ position:"absolute", inset:0, pointerEvents:"none", zIndex:0 }} aria-hidden="true">
      {PIXEL_DECO.map((p,i)=>(
        <span key={i} style={{ position:"absolute", width:7, height:7, border:`2px solid ${p.color}`, top:p.top, left:p.left, opacity:0.45, animation:`floatPx ${p.dur} ${p.delay} ease-in-out infinite` }} />
      ))}
    </div>
  );
}
...
function RankSymbol({ type, color, glow, size=54 }: { type:SymType; color:string; glow:string; size?:number }) {
  const f = `drop-shadow(0 0 4px ${glow}) drop-shadow(0 0 10px ${glow})`;
  const cr = { shapeRendering:"crispEdges" as const };
  switch (type) {
    case "novice": return (
...
function ArrowBtn({ dir, disabled, onClick }: { dir:"left"|"right"; disabled:boolean; onClick:()=>void }) {
  return (
    <button onClick={onClick} disabled={disabled} className="arrow-btn" aria-label={dir==="left"?"Previous":"Next"}
      style={{ width:44, height:150, background:"transparent", border:"none", display:"flex", alignItems:"center", justifyContent:"center", cursor:disabled?"default":"pointer", opacity:disabled?0.15:1, flexShrink:0, outline:"none", padding:0, transition:"opacity 0.15s" }}>
      <span style={{ display:"block", width:0, height:0, borderTop:"14px solid transparent", borderBottom:"14px solid transparent",
        ...(dir==="right" ? { borderLeft:`22px solid ${C.cyan}` } : { borderRight:`22px solid ${C.cyan}` }),
        filter:disabled?"none":`drop-shadow(0 0 6px ${C.cyan})`, transition:"filter 0.15s" }} />
    </button>
  );
}

// ─── PAGE 1: Current rank cinematic view ──────────────────────────────────────
    ...
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:0, width:"100%", animation:"fadeUp 0.4s both" }}>
      ...
      <div className="hero-card" style={{ position:"relative", width:"clamp(230px,52vw,460px)", background:`linear-gradient(145deg,${colorTheme.secondary} 0%,#010707 60%,#020C10 100%)`, border:`2px solid ${colorTheme.primary}`, borderRadius:6, boxShadow:`0 0 40px ${colorTheme.ring},0 0 80px ${colorTheme.ring},inset 0 0 30px rgba(0,0,0,.7)`, padding:"clamp(18px,2.8vw,32px) clamp(20px,3.5vw,44px) clamp(14px,2.2vw,26px)", display:"flex", flexDirection:"column", alignItems:"center", gap:"clamp(8px,1.3vw,14px)", overflow:"hidden" }}>
        ...
        {(["top","bottom"] as const).flatMap(v => (["left","right"] as const).map(h => (
          <span key={v+h} style={{ position:"absolute", [v]:0, [h]:0, width:18, height:18, [`border${v.charAt(0).toUpperCase()+v.slice(1)}`]:`2.5px solid ${colorTheme.glow}`, [`border${h.charAt(0).toUpperCase()+h.slice(1)}`]:`2.5px solid ${colorTheme.glow}` }} />
        )))}

        {/* Scan sweep */}
        <div style={{ position:"absolute", inset:0, background:`linear-gradient(180deg,transparent 0%,${colorTheme.ring} 50%,transparent 100%)`, animation:"scanSweep 3.5s ease-in-out infinite", pointerEvents:"none", opacity:0.15 }} />

        {/* Pulse rings */}
        {[110,86,66].map((sz,i)=>(
          <div key={i} style={{ position:"absolute", width:sz+"%", paddingTop:sz+"%", maxWidth:sz*1.8, maxHeight:sz*1.8, borderRadius:"50%", border:`1px solid ${colorTheme.ring}`, top:"50%", left:"50%", transform:"translate(-50%,-54%)", animation:`ringPulse 3.5s ${i*0.7}s ease-in-out infinite`, pointerEvents:"none", opacity:0.22 }} />
        ))}
        ...
        <div style={{ position:"relative", zIndex:2, marginTop:6 }}>
          <RankSymbol type={symbolType} color={colorTheme.primary} glow={colorTheme.glow} size={Math.min(96, Math.max(60, 78))} />
        </div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:"clamp(4px,1vw,12px)", width:"100%", maxWidth:880, padding:"0 clamp(6px,1.5vw,20px)" }}>
        ...
        <div style={{ flex:1, display:"grid", gridTemplateColumns:`repeat(${PER_SLIDE},1fr)`, gap:"clamp(8px,1.3vw,18px)" }}>
          {visible.map((rank,i)=>{
            ...
            return (
              <div key={rank.title} className="peek-card" style={{ position:"relative", background:`linear-gradient(145deg,${colorTheme.secondary} 0%,#010707 100%)`, border:`1.5px solid ${isActive?colorTheme.glow:isUnlocked?colorTheme.primary:colorTheme.primary+"44"}`, borderRadius:4, padding:"clamp(10px,1.6vw,18px) clamp(7px,1.1vw,12px) clamp(8px,1.3vw,14px)", display:"flex", flexDirection:"column", alignItems:"center", gap:"clamp(6px,0.9vw,10px)", minHeight:"clamp(140px,21vw,200px)", overflow:"hidden", transition:"transform 0.18s,box-shadow 0.18s", boxShadow:isActive?`0 0 22px ${colorTheme.ring},0 0 40px ${colorTheme.ring}`:`0 0 8px ${colorTheme.ring}`, animation:`cardIn 0.35s ${i*0.06}s both`, opacity:isUnlocked?1:0.48 }}>
                ...
                {/* Holographic scan */}
                <div style={{ position:"absolute", inset:0, background:`linear-gradient(180deg,transparent 0%,${colorTheme.ring} 50%,transparent 100%)`, animation:`scanSweep ${3+i*0.6}s ease-in-out infinite`, pointerEvents:"none", opacity:0.08 }} />
    ...
    <div style={{ position:"relative", width:"100%", minHeight:"100%", background:C.bg, display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden", fontFamily:"'Press Start 2P',monospace" }}>
      ...
      <main style={{ position:"relative", zIndex:10, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-start", width:"100%", minHeight:"100%", paddingTop:"clamp(28px,4.5vw,56px)", paddingBottom:"clamp(20px,3.5vw,44px)", gap:0 }}>
        ...
        <div style={{ width:"100%", display:"flex", justifyContent:"center", flex:1 }}>
          {page===0
            ? <CurrentRankPage totalXp={totalXp} onNext={() => { setPage(1); playSound("select"); }}/>
            : <AllRanksPage totalXp={totalXp}/>
          }
        </div>
